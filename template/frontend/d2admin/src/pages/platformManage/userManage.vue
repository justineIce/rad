<!-- 用户管理 -->
<template>
    <d2-container class="meta-public">
        <JusLayout aside :aside-width="200" title="用户管理">
            <!-- 侧面栏 -->
            <template slot="aside">
                <div class="sub-title" @click="nodeOfficeClick">组织架构</div>
                <div>
                    <!--部门列表-->
                    <jus-tree style="background-color: transparent;"
                              :data="officeData"
                              :props="officeProps"
                              icon-class="el-icon-office-building"
                              defaultExpandAll
                              @node-click="nodeOfficeClick"
                              :expand-on-click-node="false">
                        <span class="custom-tree-node" slot-scope="{ node, data }">
                            <span>{{ node.label }}</span>
                            <span class="hide_">
                                <span class="el-icon-edit mar_right"
                                      @click="() => handleEditOffice(node, data)"></span>
                                <span class="el-icon-delete" style="color: #F56C6C;"
                                      @click="() => delOffice(data.ID)"></span>
                            </span>
                        </span>
                    </jus-tree>
                </div>
                <div class="border_"></div>
                <div class="add_" @click="addOffice">
                    <span class="el-icon-plus pad"> 添加部门</span>
                </div>
            </template>

            <div class="mar_top">
                <el-row :gutter="24">
                    <div class="box_1">
                        <span class="title_">{{checkedOfficeName}}</span>
                        <span class="btn-float" v-if="checkedOfficeName!=='全部'">
                                    <el-button type="warning" @click="editOffice">
                                            <span class="el-icon-plus"></span>
                                        修改部门
                                    </el-button>
                                    <el-button type="primary" @click="addChildOffice">
                                            <span class="el-icon-plus"></span>
                                        新增子部门
                                    </el-button>
                                    <el-button type="danger" @click="delOffice(selectOfficeForm.ID)">
                                            <span class="el-icon-delete"></span>
                                        删除部门
                                    </el-button>
                                </span>
                    </div>
                    <el-form :model="searchForm" :label-width="labelWidth" label-position="left">
                        <el-col :span="4">
                            <el-form-item label="姓名：">
                                <el-input v-model="searchForm.name" placeholder="请输入姓名"></el-input>
                            </el-form-item>
                        </el-col>
                        <el-col :span="4">
                            <el-form-item label="角色：">
                                <el-select v-model="searchForm.roleId">
                                    <el-option label="不限" value=""></el-option>
                                    <el-option v-for="item in roleOption" :key="item.id" :label="item.Name"
                                               :value="item.ID"></el-option>
                                </el-select>
                            </el-form-item>
                        </el-col>
                        <el-col :span="10">
                            <el-form-item label="创建时间：" label-width="80px">
                                <div class="block">
                                    <el-date-picker
                                            v-model="searchForm.time"
                                            size="mini"
                                            type="daterange"
                                            @change="getValue"
                                            range-separator="至"
                                            start-placeholder="开始日期"
                                            end-placeholder="结束日期">
                                    </el-date-picker>
                                </div>
                            </el-form-item>
                        </el-col>
                        <el-col :span="6">
                            <el-button @click="searchDataList">查询</el-button>
                            <el-button @click="reSearchDataList">重置</el-button>
                        </el-col>
                    </el-form>
                </el-row>
            </div>

            <!--数据列表-->
            <JusTable class=""
                      :options="{'row-key':'id'}"
                      :columns="column"
                      :data="data"
                      :rowHandle="rowHandle"
                      @reset="reset"
                      @edit="edit"
                      @del="del"
                      :showPagination="true"
                      @query-data="queryData"
                      @row-data-change="handleRowDataChange">
                <template slot="header">
                    <el-button class="el-icon-circle-plus" type="primary" size="small" @click="buildUser"> 创建用户
                    </el-button>
                </template>
                <template slot="Name" slot-scope="scope">
                    <img class="header_img"
                         src="data:image/svg+xml;base64,"
                         alt="">
                    <span>{{scope.Name}}</span>
                </template>
                <template slot="Resources" slot-scope="scope">
                    <el-tag class="mar-right" size="mini" type="info" v-for="(item,index) in scope.Resources"
                            :key="index">{{item}}
                    </el-tag>
                </template>
            </JusTable>

            <!--创建用户-->
            <el-dialog :title="title" :visible.sync="buildDialogVisible">
                <el-form :model="form" :rules="formRules" ref="form" label-width="120px">
                    <el-form-item label="账号：" prop="username">
                        <el-input v-model="form.username" placeholder="请输入登录账号"></el-input>
                    </el-form-item>
                    <el-form-item label="密码：" prop="password">
                        <el-input v-model="form.password" placeholder="请输入登录密码"></el-input>
                    </el-form-item>
                    <el-form-item label="姓名：" prop="name">
                        <el-input v-model="form.name" placeholder="请输入姓名"></el-input>
                    </el-form-item>
                    <el-form-item label="手机：" prop="mobile">
                        <el-input v-model="form.mobile" placeholder="请输入手机号码"></el-input>
                    </el-form-item>
                    <el-form-item label="部门：" prop="officeId">
                        <el-cascader v-model="form.officeId" :options="officeData" :props="officeProps" :show-all-levels="true" clearable
                                     style="width: 100%"></el-cascader>
                    </el-form-item>
                    <el-form-item label="角色：" prop="roleId">
                        <el-select v-model="form.roleId" multiple>
                            <el-option v-for="item in roleOption" :key="item.id" :label="item.Name"
                                       :value="item.ID"></el-option>
                        </el-select>
                    </el-form-item>
                    <el-form-item label="邮件：" prop="email">
                        <el-input v-model="form.email"></el-input>
                    </el-form-item>
                    <el-form-item>
                        <el-button @click="submit('form')">确定</el-button>
                        <el-button @click="buildDialogVisible=false">取消</el-button>
                    </el-form-item>
                </el-form>
            </el-dialog>
            <!--添加新部门-->
            <el-dialog :title="title" :visible.sync="officeDialogVisible">
                <el-form :model="officeForm" ref="officeForm" :rules="officeFormRules" label-width="120px">
                    <el-form-item label="父级机构：" v-if="officeForm.ParentName">
                        <el-input disabled v-model="officeForm.ParentName"></el-input>
                    </el-form-item>
                    <el-form-item label="机构名称：" prop="Name">
                        <el-input v-model="officeForm.Name" placeholder="请输入机构名称"></el-input>
                    </el-form-item>
                    <el-form-item label="排序号：" prop="Sort">
                        <el-input placeholder="输入显示顺序排序" v-model="officeForm.Sort"></el-input>
                    </el-form-item>
                    <el-form-item label="机构描述：" prop="Remarks">
                        <el-input v-model="officeForm.Remarks" placeholder="请输入对机构的描述" type="textarea"
                                  :rows="3"></el-input>
                    </el-form-item>
                    <el-form-item>
                        <el-button @click="officeSubmit">保存</el-button>
                        <el-button @click="officeDialogVisible=false">取消</el-button>
                    </el-form-item>
                </el-form>
            </el-dialog>
        </JusLayout>

    </d2-container>
</template>
<script>
    import {
        AddSystemManage,
        DelSysOffice,
        DelSystemManage,
        GetAuthorityList,
        GetRoleList,
        GetSysOfficeAll,
        ResetPassword,
        SaveSysOffice
    } from '@/api/platform.manage'
    import util from '@/libs/util'

    export default {
        name: 'userManage',
        data() {
            return {
                searchForm: {
                    name: '',
                    roleId: '',
                    time: [],
                    time1: '',
                    time2: '',
                    officeId: null,
                    pageIndex: 1,
                    pageSize: 10
                },
                pagination: {
                    pageSize: 10,
                    currentPage: 0,
                    total: 0
                },
                labelWidth: '60px',
                column: [
                    {title: '姓名', key: 'Name'},
                    {title: '账号', key: 'Username'},
                    {title: '手机号', key: 'Mobile'},
                    {title: '角色', key: 'RoleName'},
                    {
                        title: '创建时间', key: 'CreatedAt',
                        formatter: (row, column, value, index) => {
                            return this.$formatDate(value)
                        }
                    },
                    {title: '邮箱', key: 'Email'}
                ],
                rowHandle: {
                    custom: [
                        {text: '重置密码', emit: 'reset'},
                        {text: '修改', emit: 'edit'},
                        {text: '删除', emit: 'del'}
                    ]
                },
                data: [],
                checkedOfficeName: '全部',
                officeProps: {
                    label: 'Name',
                    value: 'ID',
                    children: 'Children',
                    checkStrictly: true
                },
                officeData: [],
                defaultProps: {
                    children: 'children',
                    label: 'label'
                },
                buildDialogVisible: false,
                form: {
                    id: '',
                    account: '',
                    password: '',
                    username: '',
                    mobile: '',
                    roleId: '',
                    officeId: '',
                    email: '',
                    roleName: '',
                    authorityId: ''
                },
                formRules: {
                    name: [{required: true, message: '请输入用户名', trigger: 'blur'}],
                    password: [{required: true, message: '请输入密码', trigger: 'blur'}],
                    roleId: [{required: true, message: '请选择角色', trigger: 'blur'}],
                    officeId: [{required: true, message: '请选择部门', trigger: 'blur'}],
                    mobile: [
                        {required: true, message: '请输入11位数手机号', trigger: 'blur'},
                        {pattern: /^1[23456789]\d{9}$/, message: '手机号码格式错误', trigger: ['blur', 'change']}
                    ],
                    username: [{required: true, message: '请输入姓名', trigger: 'blur'}],
                    email: [
                        {type: 'email', message: '邮箱格式错误', trigger: ['blur', 'change']}
                    ]
                },
                roleOption: [],
                title: '创建用户',
                officeDialogVisible: false,
                officeForm: {
                    ID: '',
                    ParentId: '',
                    ParentName: '',
                    Name: '',
                    Remarks: '',
                    Sort: ''
                },
                officeFormRules: {
                    Name: [{required: true, message: '请输入机构名称', trigger: 'blur'}]
                },
                selectOfficeForm: null
            }
        },
        methods: {
            nodeOfficeClick(data, node, vim) {
                if (node) {
                    this.checkedOfficeName = data.Name
                    this.officeForm = {
                        ID: data.ID,
                        ParentId: data.ParentID,
                        Name: data.Name,
                        Sort: data.Sort,
                        Remarks: data.Remarks
                    }
                    this.selectOfficeForm = data
                    this.searchForm.officeId = this.officeForm.ID
                } else {
                    this.checkedOfficeName = '全部'
                    this.officeForm = {}
                    this.selectOfficeForm = null
                    this.searchForm.officeId = null
                }
                this.getDataList()
            },
            // 编辑部门
            handleEditOffice(node, data) {
                console.log(node, data)
                this.title = '编辑部门'
                this.officeDialogVisible = true
                this.officeForm = {
                    ID: data.ID,
                    ParentId: data.ParentID,
                    Name: data.Name,
                    Sort: data.Sort,
                    Remarks: data.Remarks
                }
            },
            // 删除部门
            handleRemoveOffice(node, data) {
                this.delOffice(data.ID)
            },
            // 获取角色
            getSysOfficeAll() {
                GetSysOfficeAll().then((res) => {
                    if (res.ret === 200) {
                        this.officeData = res.data
                    }
                })
            },
            // 编辑部门
            editOffice() {
                this.officeDialogVisible = true
            },
            // 添加新部门
            addOffice() {
                this.officeDialogVisible = true
                this.officeForm = {
                    ID: '',
                    ParentId: '',
                    Name: '',
                    Sort: '',
                    Remarks: ''
                }
            },
            // 添加子部门
            addChildOffice() {
                this.officeDialogVisible = true
                this.officeForm = {
                    ID: '',
                    ParentId: this.selectOfficeForm.ID,
                    ParentName: this.selectOfficeForm.Name,
                    Name: '',
                    Sort: '',
                    Remarks: ''
                }
            },
            // 删除部门
            delOffice(id) {
                this.$confirm('确定删除此部门吗？', '删除提醒', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    // 具体操作
                    DelSysOffice({id: id}).then((res) => {
                        if (res.ret === 200) {
                            this.$message.success('删除成功')
                            this.selectOfficeForm = null
                            this.checkedOfficeName = '全部'
                            this.getSysOfficeAll(0)
                        }
                    })
                }).catch(() => {
                    this.$message.warning('取消删除')
                })
            },
            officeSubmit() {
                this.$refs.officeForm.validate((valid) => {
                    if (valid) {
                        SaveSysOffice(this.officeForm).then((res) => {
                            if (res.ret === 200) {
                                this.$message.success('保存成功')
                                this.officeDialogVisible = false
                                this.getSysOfficeAll()
                            }
                        })
                    }
                })
            },
            handleRowDataChange(row) {
            },
            queryData(data, currentPage) {
                this.searchForm.pageIndex = currentPage.pageIndex
                this.searchForm.pageSize = currentPage.pageSize
                this.getDataList()
            },
            // 重置密码 - 密码忘记时
            reset({index, row}) {
                this.$confirm('确定要重置用户 ' + row.Name + ' 的密码吗？', '', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    // 具体操作
                    ResetPassword({id: row.ID}).then((res) => {
                        if (res.ret === 200) {
                            this.$message.success('重置密码成功，密码为：123456')
                        }
                    })
                }).catch(() => {
                    this.$message.warning('取消重置')
                })
            },
            // 创建时间范围
            getValue(val) {
                if (this.searchForm.time && this.searchForm.time.length > 0) {
                    this.searchForm.time1 = util.formatDate(this.searchForm.time[0], 'yyyy-MM-dd')
                    this.searchForm.time2 = util.formatDate(this.searchForm.time[1], 'yyyy-MM-dd')
                } else {
                    this.searchForm.time1 = ''
                    this.searchForm.time2 = ''
                }
            },
            // 变更角色
            edit({index, row}) {
                this.title = '编辑'
                this.buildDialogVisible = true
                this.form = {
                    id: row.ID,
                    username: row.Username,
                    name: row.Name,
                    password: '',
                    email: row.Email,
                    roleId: row.RoleId.split(','),
                    roleName: row.RoleName,
                    officeId: row.OfficeID,
                    authorityId: row.AuthorityId,
                    mobile: row.Mobile
                }
            },
            // 删除
            del({index, row}) {
                this.$confirm('确定删除用户 ' + row.Name + ' 吗？', '删除用户', {
                    confirmButtonText: '确定',
                    cancelButtonText: '取消',
                    type: 'warning'
                }).then(() => {
                    // 具体操作
                    DelSystemManage({id: row.ID}).then((res) => {
                        if (res.ret === 200) {
                            this.$message.success('删除成功')
                            this.getDataList()
                        }
                    })
                }).catch(() => {
                    this.$message.warning('取消删除')
                })
            },
            // 创建用户
            buildUser() {
                this.title = '创建用户'
                this.buildDialogVisible = true
                this.form = {
                    id: '',
                    account: '',
                    password: '',
                    username: '',
                    mobile: '',
                    roleId: '',
                    officeId: '',
                    email: '',
                    roleName: ''
                }
            },
            // 创建用户
            submit(form) {
                this.$refs[form].validate((valid) => {
                    if (valid) {
                        this.form.officeId = this.form.officeId[this.form.officeId.length - 1]
                        AddSystemManage(this.form).then((res) => {
                            if (res.ret === 200) {
                                this.$message.success('保存成功')
                                this.buildDialogVisible = false
                                this.getDataList()
                            }
                        })
                    }
                })
            },
            // 获取角色
            getRoleList() {
                GetRoleList().then((res) => {
                    if (res.ret === 200) {
                        this.roleOption = res.data
                    }
                })
            },
            // 获取数据列表
            getDataList() {
                // 这是获取有权限的管理员
                GetAuthorityList(this.searchForm).then((res) => {
                    if (res.ret === 200) {
                        var data = res.data
                        this.data = data.data
                        this.pagination = {
                            currentPage: data.pageIndex,
                            pageSize: data.pageSize,
                            total: data.Count
                        }
                    }
                })
            },
            // 查询
            searchDataList() {
                this.getDataList()
            },
            // 重置
            reSearchDataList() {
                this.searchForm = {
                    name: '',
                    roleName: '',
                    time: '',
                    time1: '',
                    time2: '',
                    pageIndex: 1,
                    pageSize: 10
                }
                this.getDataList()
            }
        },
        mounted() {
            this.getRoleList()
            this.getDataList()
            this.getSysOfficeAll()
        }
    }
</script>
<style scoped>
    .sub-title {
        padding: 10px 20px;
        margin: 20px 0 10px;
        font-weight: 700;
        font-size: 15px;
        color: #526069;
        cursor: default;
    }

    .mar_top {
        margin-top: 0px;
    }

    .mar-right {
        margin-right: 5px;
        margin-bottom: 5px;
    }

    .el-form .el-form-item {
        margin-bottom: 20px;
    }

    .header_img {
        height: 32px;
        width: 32px;
        vertical-align: middle;
        border-radius: 50%;
        margin-right: 10px;
    }

    .pad {
        margin-left: 10px;
        cursor: pointer;
        display: inline-block;
        font-size: 15px;
    }

    .add_ {
        padding: 10px 0;
        cursor: pointer;
        margin-top: 20px;
    }

    .add_:hover {
        color: #409EFF;
        background-color: #ecf5fe;
    }

    .mar_right {
        margin-right: 5px;
    }

    .title_ {
        font-size: 17px;
        font-weight: 600;
    }

    .box_1 {
        padding-bottom: 15px;
        border-bottom: 1px solid #E4E7ED;
        margin-bottom: 20px;
        overflow: hidden;
    }

    .border_ {
        height: 1px;
        border-top: 1px solid #DCDFE6;
        margin-top: 10px;
    }

    .box-content {
        height: 600px;
    }

    .box-content-first {
        height: 600px;
        overflow-y: auto;
        border-right: 1px solid #e4e7ed;
    }

    .box-btn {
        margin-top: 15px;
        padding-top: 15px;
        border-top: 1px solid #E4E7ED;
    }

    .btn-float {
        float: right;
    }

    .custom-tree-node {
        flex: 1;
        display: flex;
        align-items: center;
        justify-content: space-between;
        font-size: 14px;
        padding-right: 8px;
    }

    .hide_ {
        display: none;
    }

    .jus-tree-node__content:hover .hide_ {
        display: inline-block;
    }


</style>
