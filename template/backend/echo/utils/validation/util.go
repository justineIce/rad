...
func getValidFuncs(f reflect.StructField, alias string) (vfs []ValidFunc, err error) {
	tag := f.Tag.Get(ValidTag)
	if len(tag) == 0 {
		return
	}
	if vfs, tag, err = getRegFuncs(tag, f.Name, alias); err != nil {
		return
	}
	...
	for _, vfunc := range fs {
		...
		vf, err = parseFunc(vfunc, f.Name, alias)
		if err != nil {
			return
		}
		vfs = append(vfs, vf)
...
func getValidAliasFuncs(f reflect.StructField) (alias string, err error) {
	tag := f.Tag.Get(ValidAliasTag)
	if len(tag) == 0 {
		return
	}
	if _, alias, err = getRegFuncs(tag, f.Name, ""); err != nil {
		return
	}
	return
}
...
func getRegFuncs(tag, key, alias string) (vfs []ValidFunc, str string, err error) {
	...
	vfs = []ValidFunc{{"Match", []interface{}{reg, key + ".Match", alias}}}
	str = strings.TrimSpace(tag[:index]) + strings.TrimSpace(tag[end+len("/)"):])
	return
}

func parseFunc(vfunc, key, alias string) (v ValidFunc, err error) {
	defer func() {
		if r := recover(); r != nil {
			err = fmt.Errorf("%v", r)
		}
	...
	if start == -1 {
		...
		v = ValidFunc{vfunc, []interface{}{key + "." + vfunc, alias}}
		return
	}
