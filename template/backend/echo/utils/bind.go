	...
	for i := 0; i < typ.NumField(); i++ {
		...
		if structFieldKind == reflect.Slice && numElems > 0 {
			...
			for j := 0; j < numElems; j++ {
				if err := setWithProperType(sliceOf, inputValue[j], slice.Index(j), fmt.Sprintf("%v", typeField.Type)); err != nil {
					return err
				}
			}
			val.Field(i).Set(slice)
		} else {
			if err := setWithProperType(typeField.Type.Kind(), strings.Join(inputValue, ","), structField, fmt.Sprintf("%v", typeField.Type)); err != nil {
				return err
			}
		}
	}
...
func setWithProperType(valueKind reflect.Kind, val string, structField reflect.Value, fieldType string) error {
	...
	switch valueKind {
	case reflect.Ptr:
		return setWithProperType(structField.Elem().Kind(), val, structField.Elem(), fieldType)
	case reflect.Int:
