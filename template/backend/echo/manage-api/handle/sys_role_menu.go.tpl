	...
	if add == "true" {
		...
		if len(menuIds) > 0 {
			...
			for _, value := range menuIds {
				if err := tx.Create(&model.SysRoleMenu{ID: utils.IDString(), SysRoleID: roleID, SysMenuID: value}).Error; err != nil {
					tx.Rollback()
					global.Log.Error(err.Error())
					return utils.ErrorNull(c, tip)
				}
		...
		//删除现有资源权限以及按钮权限
		if err := tx.Delete(&model.SysRoleMenu{}, "sys_menu_id in(?) AND sys_role_id=?", menuIds, roleID).Error; err != nil {
			tx.Rollback()
			global.Log.Error(err.Error())
			return utils.ErrorNull(c, tip)
		}
		if err := tx.Delete(&model.SysRoleMenuBtn{}, "sys_menu_id in(?) AND sys_role_id=?", menuIds, roleID).Error; err != nil {
			tx.Rollback()
			global.Log.Error(err.Error())
			return utils.ErrorNull(c, tip)
		}
